...
import com.ale94.digital_banking_api.infraestructure.mappers.TransactionMapper;
                ...
                try (PDDocument document = new PDDocument(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {
                        ...
                        try (PDPageContentStream cs = new PDPageContentStream(document, page)) {
                                ...
                                float titleWidth = PDType1Font.HELVETICA_BOLD
                                                .getStringWidth("Comprobante de Transferencia") / 1000
                                                * 14;
                                ...
                                // Monto
                                startY = writeWrappedText(cs, "Monto: $" + transaction.getAmount(), marginX, startY, 12,
                                                maxWidth,
                                                lineSpacing, page);

                                // Descripción
                                if (transaction.getDescription() != null && !transaction.getDescription().isEmpty()) {
                                        startY = writeWrappedText(cs, "Descripción: " + transaction.getDescription(),
                                                        marginX, startY, 12,
                                                        maxWidth, lineSpacing, page);
                                }

                                // Desde (origen)
                                startY = writeWrappedText(cs, "Cuenta origen: " + account.getAccountNumber(), marginX,
                                                startY, 12,
                                                maxWidth, lineSpacing, page);
                                startY = writeWrappedText(cs, "Titular: " + userFrom.getName(), marginX, startY, 12,
                                                maxWidth,
                                                lineSpacing, page);

                                // Hacia (destino)
                                startY = writeWrappedText(cs, "Cuenta destino: " + transaction.getDestinationAccount(),
                                                marginX, startY,
                                                12, maxWidth, lineSpacing, page);
                                startY = writeWrappedText(cs, "Titular: " + destinationName, marginX, startY, 12,
                                                maxWidth, lineSpacing,
                                                page);

                                // Fecha
                                startY = writeWrappedText(cs, "Fecha: " + transaction.getDate(), marginX, startY, 12,
                                                maxWidth,
                                                lineSpacing, page);

                                // Número de operación
                                writeWrappedText(cs, "N° Operación: " + transaction.getOperationNumber(), marginX,
                                                startY, 12, maxWidth,
                                                lineSpacing, page);
                        }
